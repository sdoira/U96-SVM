...
{
	if ((fpga->com.InterruptStatus & FPGA_INTR_GRB_L) == FPGA_INTR_GRB_L) {
		// clear interrupt
		fpga->com.InterruptStatus = FPGA_INTR_GRB_L;

		// application specific data structure
		struct UVC_APP_DATA *app_data = Uvc_GetAppData(CallBackRef);

		// read active bank
		app_data->bank = Fpga_ReadActiveBank();
		app_data->img_received += 1;
	}
}
...
int main (void)
{
	...
	xil_printf("\r\n\r\n");
	xil_printf("/*******************************************************/\r\n");
	xil_printf("/*                Hardware Test Program                */\r\n");
	xil_printf("/*                         for                         */\r\n");
	xil_printf("/*                       U96-SVM                       */\r\n");
	xil_printf("/*                by Nu-Gate Technology                */\r\n");
	xil_printf("/*******************************************************/\r\n");
	xil_printf("FPGA ver.%08lx\r\n", Fpga_ReadVersion());
	xil_printf("%s:%s\r\n", __DATE__, __TIME__);

	xil_printf("Start USB Frame Grabber\r\n");
	App_UsbGrabber();
...
void CommandExecute (
	...
	if (strcmp(cmd.parm[0], "ol") == 0) {
		...
		*ptr = (unsigned char)(data & 0xFF);
		...
		fpga->grb_l.Address_A = 0x40000000;
...
//=============================================================================
void App_UsbGrabber (void) {
	s32 Status;

	// Initialize FPGA
	Fpga_Init(IMG_BUF_A, IMG_BUF_B, IMG_BUF_C);
	...
	// Start
	xil_printf("Start USB driver (Press ESC to leave)\r\n");
	Xusb_Main();
}
...
void App_9DofHwTest (void) {
	...
	for (int ch = 0; ch < 2; ch++) {
		...
		if (found == 1) {
			...
			while(1) {
				if (ch == 0) {
					gpio = fpga->com.GPIO & 0x00000001;
				} else {
					gpio = (fpga->com.GPIO >> 1) & 0x00000001;
				}
